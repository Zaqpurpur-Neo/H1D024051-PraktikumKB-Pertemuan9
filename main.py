...
from InsiasiPopulasi import inisialisasi_populasi
from mutation import inversion_mutation, swap_mutation, uniform_mutation
...
def run_ga(
    jumlah_generasi, jumlah_populasi, prob_crossover, prob_mutasi, kapasitas_tas
):
    ...
    fitness_populasi = [
        hitung_fitness(individu, barang, kapasitas_tas) for individu in populasi
    ]
    ...
    for generasi in range(jumlah_generasi):
        ...
        while len(new_populasi) < jumlah_populasi:
            ...
            available_indices = [
                i for i in range(len(populasi)) if i not in used_indices
            ]
    ...
    selected_items = [
        barang[i][0] for i in range(len(best_individu)) if best_individu[i] == 1
    ]
    selected_value = hitung_fitness(best_individu, barang, kapasitas_tas)
    selected_weight = sum(
        [barang[i][2] for i in range(len(best_individu)) if best_individu[i] == 1]
    )
